...
uint16 MUS_OFFSET, INS_OFFSET, SFX_OFFSET, BUZ_OFFSET;
uint8 SONG_COUNT, SFX_COUNT;
uint8 AUDIOVERSION, AUDIOTYPE;
bool FX_ON;
uint16 FX_TIME;
uint8 AUDIOTIMING;
int lastaudiotick;
...
extern unsigned int loaduint16(unsigned char c1, unsigned char c2);
extern int loadint16(unsigned char c1, unsigned char c2);
...
#else
	#ifdef NSPIRE
		sprintf(path_work, "/documents/opentitus/%s.tns", filename);
...
#else
	#ifdef NSPIRE
		sprintf(path_work, "/documents/opentitus/%s.tns", audiofile);
	#elif defined(DREAMCAST)
