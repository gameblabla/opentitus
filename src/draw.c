...
#ifdef malloc
    *pixelformat = (SDL_PixelFormat *)malloc(sizeof(SDL_PixelFormat));
    if (*pixelformat == NULL) {
        sprintf(lasterror, "Error: Not enough memory to initialize palette!\n");
        return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
    }

    (*pixelformat)->palette = (SDL_Palette *)malloc(sizeof(SDL_Palette));
    ...
    (*pixelformat)->palette->colors = (SDL_Color *)malloc(sizeof(SDL_Color) * (*pixelformat)->palette->ncolors);
