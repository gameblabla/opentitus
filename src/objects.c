...
int shock(TITUS_level *level, TITUS_object *object);

int move_objects(TITUS_level *level) {

    if (GRAVITY_FLAG == 0) return 1; //Skip execution if there are no active objects
...
int shock(TITUS_level *level, TITUS_object *object) { //Falling object versus player
    ...
    //Quick test
    if (object->mass < 10) return 1;
    if (player->sprite.speedY >= MAX_Y*16) return 1;
    ...
    if (object->sprite.x > player->sprite.x) { //Object center is right for player
        if (object->sprite.x > player->sprite.x + 24) return 1; //Object is too far right
    } else { //Object center is left for center
        if (object->sprite.x + object->sprite.spritedata->collwidth < player->sprite.x) return 1; //Return if object is too far left
    }    
    
    //Test Y
    
    if (object->sprite.y < player->sprite.y) { //Object bottom is above player bottom
        if (object->sprite.y <= player->sprite.y - 32) return 1; //Return if object is completely above the player
    } else { //Object bottom is below player bottom
        if (object->sprite.y - object->sprite.spritedata->collheight + 1 >= player->sprite.y) return 1; //Return if object is completely below the player
    }
...
#else
    *objects = (TITUS_objectdata **)malloc(sizeof(TITUS_objectdata *) * ORIG_OBJECT_COUNT);
#endif
    if ((*objects) == NULL) {
...
#else
        (*objects)[i] = (TITUS_objectdata *)malloc(sizeof(TITUS_objectdata)); // * ORIG_OBJECT_COUNT);
#endif
        if ((*objects)[i] == NULL) {
            sprintf(lasterror, "Error: Not enough memory to load objects!\n");
            return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
