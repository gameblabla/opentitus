...
extern int FX_START(int fx_number);
extern int RETURN_MUSIC();
...
int ARAB_TOMBE(TITUS_level *level);
int CASE_DEAD_IM (TITUS_level *level);
int DEC_LIFE (TITUS_level *level);
int t_pause (TITUS_level *level);
int BRK_COLLISION(TITUS_level *level);
int ARAB_BLOCKX(TITUS_level *level);
int ARAB_BLOCK_YU(TITUS_player *player);
int DECELERATION(TITUS_player *player);
int ARAB_TOMBE_F();
...
// TODO: ADD!    SCREEN_3(); //Test for hidden credits screen
    ...
    //Part 2: Determine the player's action, and execute action dependent code
    X_FLAG = state->buttons & CONT_DPAD_LEFT | state->buttons & CONT_DPAD_RIGHT; //Set if either is true
    Y_FLAG = state->buttons & CONT_DPAD_UP | state->buttons & CONT_A | state->buttons & CONT_DPAD_DOWN; //Set if either is true
    if (NOCLIP) {
        ...
        if ((state->buttons & CONT_DPAD_UP) || (state->buttons & CONT_A)) {
    ...
    else if (KICK_FLAG != 0) 
    {
        ...
        } else if (!PRIER_FLAG && (state->buttons & CONT_DPAD_UP || state->buttons & CONT_A) && (SAUT_FLAG == 0)) {
        ...
        } else if (PRIER_FLAG || ((SAUT_FLAG != 6) && (state->buttons & CONT_DPAD_DOWN))) {
        ...
        if ((state->buttons & CONT_B) && !PRIER_FLAG) {
            if (!DROP_FLAG) {
    ...
    if ((CHOC_FLAG != 0) || (KICK_FLAG != 0)) {
        ...
        //Is space button pressed?
        if ((keystate[KEY_SPACE] || keystate[KEY_HOLD]) && !PRIER_FLAG) {
            if (!DROP_FLAG) {
                if ((action == 3) || (action == 5)) { //Kneestand
                    DROPREADY_FLAG = false;
                    action = 7; //Grab an object
...
int BRK_COLLISION(TITUS_level *level) { //Collision detection between player and tiles/objects/elevators
...
#ifdef DREAMCAST
            if ((state->buttons & CONT_DPAD_UP || state->buttons & CONT_A) && (order == 6)) { //action UP + climb ladder
#else
...
int COLLISION_OBJET(TITUS_level *level) {
    ...
    if (!(SPRITES_VS_SPRITES(level, &(player->sprite), (TITUS_sprite *)&(level->spritedata[0]), &off_object))) { //check if player stands on an object, use sprite[0] (rest) as collision size (first player tile)
        return 1;
    }
