...
#else
    level->tilemap = (unsigned char **)malloc(sizeof(unsigned char *) * pre_size);
#endif
...
#else
        level->tilemap[i] = (unsigned char *)malloc(sizeof(unsigned char) * level->width);
#endif 
...
#else
    level->object = (TITUS_object *)malloc(sizeof(TITUS_object) * pre_size);
#endif
...
#else
    level->enemy = (TITUS_enemy *)malloc(sizeof(TITUS_enemy) * pre_size);
#endif
    if (level->enemy == NULL) {
        sprintf(lasterror, "Error: Not enough memory to initialize level!\n");
        return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
...
#else
    level->bonus = (TITUS_bonus *)malloc(sizeof(TITUS_bonus) * level->bonuscount);
#endif
    if (level->bonus == NULL) {
        sprintf(lasterror, "Error: Not enough memory to initialize level!\n");
        return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
...
#else
    level->gate = (TITUS_gate *)malloc(sizeof(TITUS_gate) * level->gatecount);
#endif
    if (level->gate == NULL) {
        sprintf(lasterror, "Error: Not enough memory to initialize level!\n");
        return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
...
#else
    level->elevator = (TITUS_elevator *)malloc(sizeof(TITUS_elevator) * pre_size);
#endif
...
#else
    level->trash = (TITUS_sprite *)malloc(sizeof(TITUS_sprite) * pre_size);
#endif
    if (level->trash == NULL) {
        sprintf(lasterror, "Error: Not enough memory to initialize level!\n");
        return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
