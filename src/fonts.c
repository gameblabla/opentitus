...
int loadfonts(void) {
    ...
    //First byte
    font = (TITUS_font *)malloc(sizeof(TITUS_font) * 256);
    if (font == NULL) {
...
/*    
    //Second byte
    font->sub[0] = (TITUS_font *)malloc(sizeof(TITUS_font) * 256);
    if (font->sub[0] == NULL) {
        sprintf(lasterror, "Error: Not enough memory to load fonts!\n");
        freepixelformat(&(pixelformat));
        return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
    ...
    //Third byte
    f_sub->sub[0] = (TITUS_font *)malloc(sizeof(TITUS_font) * 256);
    if (f_sub->sub[0] == NULL) {
        sprintf(lasterror, "Error: Not enough memory to load fonts!\n");
        freepixelformat(&(pixelformat));
        return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
...
int SDL_Print_Text(char *text, int x, int y){
    ...
    for (i = 0; i < strlen(text); i++) {
        ...
        case 0: //Undefined character
            ...
            //If first letter is larger than 10111111: this is multibyte. Make sure "i" is in the last byte
            /*if (text[j] > 0xBF) 
            { 
                do {
                    if (j == strlen(text) - 1) {
