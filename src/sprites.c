...
int animate_sprite(TITUS_level *level, TITUS_sprite *spr);
...
#else
    *sprites = (TITUS_spritedata **)malloc(sizeof(TITUS_spritedata *) * SPRITECOUNT);
#endif
    if ((*sprites) == NULL) {
...
#else
        (*sprites)[i] = (TITUS_spritedata *)malloc(sizeof(TITUS_spritedata) * 2); //For some reason, it requires some more space in order to free properly
#endif
        if ((*sprites)[i] == NULL) {
            sprintf(lasterror, "Error: Not enough memory to load sprites!\n");
            return (TITUS_ERROR_NOT_ENOUGH_MEMORY);
...
#else
	spritecache->spritebuffer = (TITUS_spritebuffer **)malloc(sizeof(TITUS_spritebuffer *) * spritecache->count);
#endif
...
#else
        spritecache->spritebuffer[i] = (TITUS_spritebuffer *)malloc(sizeof(TITUS_spritebuffer));
#endif
...
int animate_sprite(TITUS_level *level, TITUS_sprite *spr) {
    if (!spr->visible) return 1; //Not on screen?
    if (!spr->enabled) return 1;
    if (spr->number == (FIRST_OBJET+26)) { //Cage
        ...
        if ((IMAGE_COUNTER & 0x0001) == 0) { //Every 2
            if (spr->UNDER == 0) {
                return 1; //Spring is not loaded; remain big
            } else if (GRAVITY_FLAG > 1) { //if not gravity, not clear
                spr->UNDER = 0;
            } else {
                spr->UNDER = spr->UNDER & 0x01; //Keep eventually object load, remove player load
